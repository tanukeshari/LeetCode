...
class Solution {
    public void nextPermutation(int[] nums) {
        ...
        for (int i = len - 1; i > 0; i--) {
            if (nums[i - 1] < nums[i]) {
                int idx = findSmallestBigger(nums, i - 1, len - 1);
    ...
    private int findSmallestBigger(int[] nums, int cur, int end) {
        int target = nums[cur];
        int idx = 0;
...
// Success
// Details 
// Runtime: 1 ms, faster than 83.13% of Java online submissions for Next Permutation.
// Memory Usage: 43.5 MB, less than 70.11% of Java online submissions for Next Permutation.
